...
# Create reverse mapping for fertilizer codes
...
with col2:   
    ...
    # Submit button
    submitted = st.button("Predict Fertilizer")
...
if submitted:
    try:
        ...
        # Scale the input data (use the saved scaler)
        input_scaled = scaler.transform(input_data)

        # Predict the fertilizer probabilities
        probabilities = model.predict_proba(input_scaled)[0]

        # Get the top 3 fertilizer indices and their probabilities
        top_3_indices = np.argsort(probabilities)[::-1][:3]
        top_3_probabilities = probabilities[top_3_indices]
        top_3_fertilizers = [reverse_fertilizer_mapping[i] for i in top_3_indices]

        # Display the result
        st.success(f"The top 3 recommended fertilizers for the selected crop ({crop_name}) are:")
        for i in range(3):
            st.write(f"{i+1}. **{top_3_fertilizers[i]}** with probability: {top_3_probabilities[i]:.2%}")
